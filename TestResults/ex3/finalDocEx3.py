...
11: nUnion
"""
results_ex1 = read_file("testResultsTrivialex2.log")
results_ex1 = results_ex1[np.lexsort((results_ex1[:,0], results_ex1[:,2]))]
results_ex1_vals = results_ex1[:,3:].astype(int)

results_all = read_file("testResultsB6I4.log")
results_all = results_all[np.lexsort((results_all[:,0], results_all[:,2]))]
results_labels = results_all[:,0:3]
results_values_und = results_all[:,3:].astype(int)
results_values = results_all[:,3:].astype(int) - results_ex1_vals
...
plt.title('Difference in synchronization times')
plt.show()
fig.savefig('syncTimes_EX3.png')
...
ax1.set_xlabel("Test cases")
plt.title('Difference in bytes transmitted')
plt.show()
fig.savefig('bytes_EX3.png')

fig = plt.figure(1)
ax1 = fig.add_subplot()
for i in range(len(results_all_unique_configs)):
    idxs = results_labels[:,2] == results_all_unique_configs[i]
    arr = results_values_und[idxs,:]
    uniques = np.unique(results_labels[idxs,0], return_index=True)
    arr2 = np.asarray(np.split(arr[:,6], uniques[1][1:]))
    ax1.bar(x_pos + i * bar_width,  np.mean(arr2, axis=1), label=results_all_unique_configs[i],
            width = bar_width, yerr=np.std(arr2, axis=1))
...
plt.show()
fig.savefig('syncTimes2_EX3.png')
